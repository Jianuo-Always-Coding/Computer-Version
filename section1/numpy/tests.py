...
def t2(X):
    ...
    """
    # A is the Eigenvalues of X, B is the eigenvecors of X
    A, B = np.linalg.eig(X)
    smallest_index = np.argmin(A)
    return B[:, smallest_index]
...
def t9(q, k, v):
    ...
    """
    result = np.exp(-np.sum((q - k) * (q - k), axis=1, keepdims=True))
    result = result * v
...
def t12(X, Y):
    ...
    result = norm_X + norm_Y.T - 2 * np.dot(X, Y.T)
    result = np.sqrt(np.maximum(0, result))
    return result
...
def t18(N, r, x, y):
    ...
    """
    a = np.ones((N, N))
    # the arange returns 1D
    S = np.arange(N).reshape(1, -1) 
    A = a * S
    B = a * (S.T)
    result = np.sqrt((A - x) * (A - x) + (B - y) * (B - y)) < r
...
def t19(N, s, x, y):
    ...
    A = S * np.ones((N, N))
    B = S.T * np.ones((N, N))
    result = np.exp(-((A - x) * (A - x) + (B - y) * (B - y)) / (s * s))
    return result.astype(float)
